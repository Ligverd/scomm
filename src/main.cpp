...
void *Client_ptread(void* arg)
{
	...
	Loger("Client","connected! IP:",Client[i]->Get_ip_str(),i);
	struct timeval tv;
	fd_set fds;
	int retval;
	...
	Loger("Client","disconnected! IP:",Client[i]->Get_ip_str(),i);
	delete Client[i];
	Client[i] = NULL;
				...
				else//Esli kanal rabochij dobavliajem novogo klienta
				{
					...
					else
					{
						...
						if (i < MAX_CLIENT) 
						{
							...
							{
								Loger("Can't create tread for client","IP:",Client[i]->Get_ip_str(),i);
								delete Client[i];
							}
						}
...
int main(int argc, char **argv)
{
	...
	if (argc == 4 || argc == 5 )
	{
		...
		{
			COMMn = atoi(argv[2]);
			if ( COMMn>MAX_COMM_PORT_No || check_d(argv[2])<0)
			{
		...
		else
		{
			...
			if (ATS_PORT<MIN_ATS_PORT || ATS_PORT>65535 || check_d(argv[2])<0)
			{
				printf("ATS port error!\r\n");
				printf("ATS port example:X\r\n");
				printf("N = 1025..65535\r\n");
				printf("If comm port n = 0..10\r\n");
				ext = true;
			}
		}
		scomm_server_port = atoi(argv[3]);
		if (scomm_server_port<MIN_SCOMM_SERVER_PORT || scomm_server_port>65535 || check_d(argv[3])<0)
		{
			printf("Scomm server port error!\r\n");
			printf("Scomm server port example:X\r\n");
	...
	printf("\r\n");
	printf("<---------------------------------------Scomm_v0.2.10------------------------------------------->\r\n");

	if ((logfilefd = Open_log_file("Scommlog"))<=0) printf("Can't create logfile!\r\n");
